...
ipc.send("log", "info", "Renderer process started!");
let remote;
try {
    remote = require("@electron/remote");
    ipc.send("log", "info", "Remote module required successfully");
} catch (e) {
    ipc.send("log", "error", "Failed to require remote: " + e.message);
}

// exposing for eval-based actions
...
// Init audio
ipc.send("log", "info", "Initializing AudioManager...");
try {
    window.audioManager = new AudioManager();
    ipc.send("log", "info", "AudioManager initialized.");
} catch (e) {
    ipc.send("log", "error", "AudioManager failed: " + e.message);
    // Stub it so app doesn't crash
    window.audioManager = {
        granted: { play: () => { } },
        stdout: { play: () => { } },
        theme: { play: () => { } },
        ...
        keyboard: { play: () => { } },
        panel: { play: () => { } },
        panels: { play: () => { } }, // Fix typo causing potential issues
        scan: { play: () => { } },
        denied: { play: () => { } },
        info: { play: () => { } },
        alarm: { play: () => { } },
...
window.openSettings = async () => {
    ...
    new Modal({
        ...
        buttons: [
            ...
            { label: "Restart eDEX", action: "const remote = require('@electron/remote'); remote.app.relaunch();remote.app.quit();" }
        ]
    }, () => {
